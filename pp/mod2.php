...
if (defined('STDIN')) {
  if (isset($argv[1])) {
    switch ($argv[1]) {
      ...
      case 'help':
        echo "
      \"limit\": n; isi (n) dengan angka (untuk menentukan maksimal eksekusi)\n
      \"loop\": n; isi (n) dengan angka (untuk menentukan berapa kali setiap eksekusi)\n
...
{
  global $cfg;
  $arr = call_user_func($cfg['header']);
  $url = 'https://www.paypal.com/myaccount/money/api/currencies/transfer';
  $h = explode("\n", str_replace($arr, '', "Cookie: $cookie
	Content-Type: application/json
	user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.130 Safari/537.36"));
  $body = call_user_func($cfg['body'], $csrf);
  $x = call_user_func($cfg['curl'], $url, $h, $body);
...
function b1($csrf)
{
  return "{\"sourceCurrency\":\"JPY\",\"sourceAmount\":2,\"targetCurrency\":\"TWD\",\"_csrf\":\"$csrf\"}";
}
 ...
 * @return json_decode
 */
function twd2usd($cookie, $csrf, $max, $sleep = false)
{
 ...
 * @return json_decode
 */
function usd2twd($cookie, $csrf, $max, $sleep = false)
{
 ...
 * @return json_decode
 */
function jpy2twd($cookie, $csrf, $max, $sleep = false)
{
